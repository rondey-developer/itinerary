...
<head>
  ...
  <title>東京 & 草津溫泉 浪漫雙人之旅 | 互動路線地圖</title>
...
<body>
  ...
  <script>
    ...
    // Real Traffic & Road Paths (Following actual roads & rails)
    const trafficRoutes = {
      1: [
        [35.6015, 139.5165],
        [35.6025, 139.5120],
      ...
      const balloonHtml = `
        <div class="popup-inner">
          ...
          <div class="popup-button-row">
            <a href="${spot.link}" target="_blank" class="popup-btn btn-detail">店家官網/預約</a>
    ...
    function renderSolidTrafficLines(day) {
      ...
      if (day === 'all') {
        [1, 2, 3, 4, 5].forEach(d => {
          ...
          const glow = L.polyline(coords, { color: '#ffffff', weight: 6, opacity: 0.85 }).addTo(map);
          const mainLine = L.polyline(coords, { color: dayColors[d], weight: 3.5, opacity: 0.95 }).addTo(map);
          activeRoutePolylines.push(glow, mainLine);
        });
      } else {
        const coords = trafficRoutes[day];
        const glow = L.polyline(coords, { color: '#ffffff', weight: 7, opacity: 0.95 }).addTo(map);
        const mainLine = L.polyline(coords, { color: dayColors[day] || '#9e2846', weight: 4, opacity: 1.0 }).addTo(map);
        activeRoutePolylines.push(glow, mainLine);
      }
    }

    // VEHICLE TRAFFIC ANIMATION
    function startMovingTraffic(day) {
      ...
      let step = 0;
      const totalSteps = targetPath.length * 35;
    ...
    // When clicked from Map Controls or Accordion
    function selectDayFromMap(day, btnEl) {
      ...
      // EXPAND AND HIGHLIGHT IN DRAWER
      ...
      if (day !== 'all') {
        ...
        if (targetBlock) {
          // ALWAYS EXPAND THE SELECTED DAY!
        ...
        // Expand all when 'all' is selected
        document.querySelectorAll('.day-block').forEach(b => {
          b.classList.remove('collapsed');
          const txt = b.querySelector('.accordion-chevron-text');
          if (txt) txt.textContent = '收合';
    ...
    function fitFullTrafficRoute() {
      selectDayFromMap('all', document.querySelector('.day-selector-btn:first-child'));
...
print("Expandable Accordion HTML generated successfully!")
